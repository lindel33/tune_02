...
def get_current_product():
    sql = "SELECT tune_admin_product.series_id, tune_admin_seriescategory.category " \
          " FROM tune_admin_product, tune_admin_seriescategory" \
          " WHERE tune_admin_product.series_id = tune_admin_seriescategory.id AND sell != 1;"
    cursor.execute(sql)
    result = cursor.fetchall()
    return result
